...
-   **Server**: The `hccr start` command launches a server that listens for requests from Claude Code. The server logic is initiated from `src/index.ts`.
...
-   **Claude Code Integration**: When a user runs `hccr code`, the command is forwarded to the running router service. The service then processes the request, applies routing rules, and sends it to the configured LLM. If the service isn't running, `hccr code` will attempt to start it automatically.
